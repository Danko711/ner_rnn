...
class CRFDevice(nn.Module):
    def __init__(self, nb_labels, bos_tag_id, eos_tag_id, device, batch_first=True):
        super().__init__()
        ...
        self.device = device
        self.transitions = nn.Parameter(torch.empty(self.nb_labels, self.nb_labels, device=self.device))
        self.init_weights()
    ...
    def log_likelihood(self, emissions, tags, mask=None):
        ...
        if mask is None:
            mask = torch.ones(emissions.shape[:3], dtype=torch.float, device=self.device)

        scores = self._compute_scores(emissions, tags, mask)
        partitions = self._compute_log_partitions(emissions, mask)
        s = torch.sum(scores - partitions)
        return s
    ...
    def _compute_log_partitions(self, emissions, mask):

        batch_size, seq_len, nb_labels = emissions.shape
    ...
    def _viterbi_decode(self, emissions, mask):
        ...
        best_sequences = []
        emission_len = mask.int().sum(dim=1)

        for i in range(batch_size):
            sample_length = emission_len[i].item()
